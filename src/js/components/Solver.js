...
class Solver {
    ...
    this.game.cube.pieces.forEach( ( piece, index ) => {
      ...
      const position = this.game.controls.getPiecePosition( piece );
      const edges = piece.userData.edges;

      if ( position.equals( vector.set(  0,  1,  1 ) ) ) positions[ 0 ] = edges[ 0 ] + edges[ 1 ];
      if ( position.equals( vector.set(  1,  1,  0 ) ) ) positions[ 1 ] = edges[ 1 ] + edges[ 0 ];
      if ( position.equals( vector.set(  0,  1, -1 ) ) ) positions[ 2 ] = edges[ 0 ] + edges[ 1 ];
      if ( position.equals( vector.set( -1,  1,  0 ) ) ) positions[ 3 ] = edges[ 1 ] + edges[ 0 ];

      if ( position.equals( vector.set(  0, -1,  1 ) ) ) positions[ 4 ] = edges[ 0 ] + edges[ 1 ];
      if ( position.equals( vector.set(  1, -1,  0 ) ) ) positions[ 5 ] = edges[ 1 ] + edges[ 0 ];
      if ( position.equals( vector.set(  0, -1, -1 ) ) ) positions[ 6 ] = edges[ 0 ] + edges[ 1 ];
      if ( position.equals( vector.set( -1, -1,  0 ) ) ) positions[ 7 ] = edges[ 1 ] + edges[ 0 ];

      if ( position.equals( vector.set(  1,  0,  1 ) ) ) positions[ 8 ] = edges[ 1 ] + edges[ 0 ];
      if ( position.equals( vector.set( -1,  0,  1 ) ) ) positions[ 9 ] = edges[ 1 ] + edges[ 0 ];
      if ( position.equals( vector.set(  1,  0, -1 ) ) ) positions[ 10 ] = edges[ 1 ] + edges[ 0 ];
      if ( position.equals( vector.set( -1,  0, -1 ) ) ) positions[ 11 ] = edges[ 1 ] + edges[ 0 ];

      if ( position.equals( vector.set(  1,  1,  1 ) ) ) positions[ 12 ] = edges[ 1 ] + edges[ 2 ] + edges[ 0 ];
      if ( position.equals( vector.set(  1,  1, -1 ) ) ) positions[ 13 ] = edges[ 1 ] + edges[ 0 ] + edges[ 2 ];
      if ( position.equals( vector.set( -1,  1, -1 ) ) ) positions[ 14 ] = edges[ 1 ] + edges[ 2 ] + edges[ 0 ];
      if ( position.equals( vector.set( -1,  1,  1 ) ) ) positions[ 15 ] = edges[ 1 ] + edges[ 0 ] + edges[ 2 ];

      if ( position.equals( vector.set(  1, -1,  1 ) ) ) positions[ 16 ] = edges[ 1 ] + edges[ 0 ] + edges[ 2 ];
      if ( position.equals( vector.set( -1, -1,  1 ) ) ) positions[ 17 ] = edges[ 1 ] + edges[ 2 ] + edges[ 0 ];
      if ( position.equals( vector.set( -1, -1, -1 ) ) ) positions[ 18 ] = edges[ 1 ] + edges[ 0 ] + edges[ 2 ];
      if ( position.equals( vector.set(  1, -1, -1 ) ) ) positions[ 19 ] = edges[ 1 ] + edges[ 2 ] + edges[ 0 ];
	...
	setState( cube ) {
		...
		for(var i=0; i<20; i++){
			...
			while((this.currentState[i] = goal.indexOf(cubie)) == -1){
				console.log( cubie, goal.indexOf(cubie), i )
	...
	verifyState() {
		...
		let sum = 0;
		this.currentState.slice(20,32).forEach(edge => {
			sum+=edge;
		});
		if(sum % 2 != 0){
			//edge orientation
		...
		this.currentState.slice(32,40).forEach(edge => {
			sum+=edge;
		});
		if(sum % 3 != 0){
			//corner orientation
	...
	solveAsync( cube, callback, progress ) {
		this.solution = '';
		this.phase = 1;
		if(cube){
			if(!this.setState(cube)){
